 ...
 */
const useClusterQueueDetails = (clusterQueueName?: string): UseClusterQueueDetailsResult => {
  const {
    data: clusterQueues,
  ...
  const clusterQueue = React.useMemo(() => {
    ...
    const matchingClusterQueue = clusterQueues.find((cq) => cq.metadata?.name === clusterQueueName);
