...
export const getTrainingProgress = (job: TrainingJob): number => {
  ...
  if (
    (job as any).status?.conditions?.some(
      (c: any) => (c.type === 'Succeeded' || c.type === 'Complete') && c.status === 'True',
    )
  ) {
    return 100;
  }
...
const getBasicTrainJobStatus = (job: TrainJobKind): TrainJobState => {
  ...
  const completeCondition = sortedConditions.find(
    (c) => c.type === 'Complete' && c.status === 'True',
  );
  const failedCondition = sortedConditions.find((c) => c.type === 'Failed' && c.status === 'True');
  const suspendedCondition = sortedConditions.find(
    (c) => c.type === 'Suspended' && c.status === 'True',
  );
  const resumedCondition = sortedConditions.find(
    (c) => c.type === 'Suspended' && c.status === 'False' && c.reason === 'Resumed',
  );
  ...
  if (job.status.jobsStatus && job.status.jobsStatus.length > 0) {
    ...
    if (
      job.status.jobsStatus.every((jobStatus) => jobStatus.succeeded > 0 && jobStatus.active === 0)
    ) {
      return TrainJobState.COMPLETE;
    }
...
export const getJobStatusWithHibernationGeneric = async (
  job: TrainingJob,
