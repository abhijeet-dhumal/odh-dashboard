...
const WorkerNodesIcon: React.FC<WorkerNodesIconProps> = ({ job }) => {
  ...
  const getWorkerNodeInfo = React.useMemo(() => {
    if (job.kind === 'PyTorchJob') {
      ...
      const workerResources =
        pytorchJob.spec.pytorchReplicaSpecs.Worker?.template?.spec?.containers?.[0]?.resources;
      const masterResources =
        pytorchJob.spec.pytorchReplicaSpecs.Master?.template?.spec?.containers?.[0]?.resources;
  ...
  const getResourcesPerProcess = React.useMemo(() => {
    ...
    const procPerNode =
      typeof numProcPerNode === 'string' ? parseInt(numProcPerNode, 10) : numProcPerNode;
    ...
    const calculatePerProcess = (resource: string | undefined, processes: number): string => {
      ...
      // Add type guard to prevent TypeError when resource is undefined
      if (typeof resource === 'string' && resource.endsWith('m')) {
        const millicores = parseInt(resource.slice(0, -1), 10);
        const coresPerProcess = millicores / processes / 1000;
        return coresPerProcess >= 1
          ? `${coresPerProcess.toFixed(1)}`
          : `${Math.round(coresPerProcess * 1000)}m`;
      }
      if (!isNaN(parseFloat(resource))) {
      ...
      // Add type guard to prevent TypeError when resource is undefined
      if (typeof resource === 'string' && resource.match(/^\d+(\.\d+)?[KMGTPE]?i?$/)) {
    ...
    >
      ...
      <div style={{ marginBottom: '12px' }}>
        ...
        <div style={{ fontSize: '12px', marginBottom: '3px' }}>
          <span style={{ color: '#4da6ff', fontWeight: '600' }}>Nodes:</span>{' '}
          {getWorkerNodeInfo.numNodes}
        </div>
        <div style={{ fontSize: '12px', marginBottom: '3px' }}>
          <span style={{ color: '#4da6ff', fontWeight: '600' }}>Processes per node:</span>{' '}
          {getWorkerNodeInfo.numProcPerNode}
        </div>
        <div style={{ fontSize: '12px', marginBottom: '3px' }}>
          <span style={{ color: '#4da6ff', fontWeight: '600' }}>Total processes:</span>{' '}
          {getWorkerNodeInfo.numNodes *
            (typeof getWorkerNodeInfo.numProcPerNode === 'string'
              ? parseInt(getWorkerNodeInfo.numProcPerNode, 10)
              : getWorkerNodeInfo.numProcPerNode)}
        </div>
        {getWorkerNodeInfo.jobType === 'PyTorchJob' &&
          getWorkerNodeInfo.workerNodes !== undefined && (
            <>
              <div style={{ fontSize: '12px', marginBottom: '3px' }}>
                <span style={{ color: '#4da6ff', fontWeight: '600' }}>Worker nodes:</span>{' '}
                {getWorkerNodeInfo.workerNodes}
              </div>
              <div style={{ fontSize: '12px', marginBottom: '3px' }}>
                <span style={{ color: '#4da6ff', fontWeight: '600' }}>Master nodes:</span>{' '}
                {getWorkerNodeInfo.masterNodes}
              </div>
            </>
          )}
      </div>
      ...
      <div style={{ marginBottom: '12px' }}>
        ...
        <div style={{ fontSize: '12px', marginBottom: '3px' }}>
          <span style={{ color: '#4da6ff', fontWeight: '600' }}>CPU Requests:</span>{' '}
          {getWorkerNodeInfo.resourcesPerNode.requests?.cpu || 'N/A'}
        </div>
        <div style={{ fontSize: '12px', marginBottom: '3px' }}>
          <span style={{ color: '#4da6ff', fontWeight: '600' }}>CPU Limits:</span>{' '}
          {getWorkerNodeInfo.resourcesPerNode.limits?.cpu || 'N/A'}
        </div>
        <div style={{ fontSize: '12px', marginBottom: '3px' }}>
          <span style={{ color: '#4da6ff', fontWeight: '600' }}>Memory Requests:</span>{' '}
          {getResourcesPerProcess.memoryRequests}
        </div>
        <div style={{ fontSize: '12px', marginBottom: '3px' }}>
          <span style={{ color: '#4da6ff', fontWeight: '600' }}>Memory Limits:</span>{' '}
          {getResourcesPerProcess.memoryLimits}
        </div>
        {getResourcesPerProcess.gpus > 0 && (
          <div style={{ fontSize: '12px', marginBottom: '3px' }}>
            <span style={{ color: '#4da6ff', fontWeight: '600' }}>GPUs per node:</span>{' '}
            {getResourcesPerProcess.gpus}
          </div>
        )}
      </div>
      ...
      {getWorkerNodeInfo.numProcPerNode > 1 && (
        <div style={{ marginBottom: '8px' }}>
          ...
          <div style={{ fontSize: '12px', marginBottom: '3px' }}>
            <span style={{ color: '#4da6ff', fontWeight: '600' }}>CPU Requests:</span>{' '}
            {getResourcesPerProcess.cpuRequests}
          </div>
          <div style={{ fontSize: '12px', marginBottom: '3px' }}>
            <span style={{ color: '#4da6ff', fontWeight: '600' }}>CPU Limits:</span>{' '}
            {getResourcesPerProcess.cpuLimits}
          </div>
        </div>
      )}
