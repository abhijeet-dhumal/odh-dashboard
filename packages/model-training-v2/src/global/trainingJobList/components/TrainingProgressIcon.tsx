...
const TrainingProgressIcon: React.FC<TrainingProgressIconProps> = ({ job }) => {
  ...
  const isSuspended = React.useMemo(() => {
    if (job.kind === 'TrainJob') {
      ...
      return (job as any).status?.conditions?.some(
        (c: any) => c.type === 'Suspended' && c.status === 'True',
      );
    }
  ...
  const tooltipContent: React.ReactNode = (
    ...
    >
      ...
      >
        ...
        (progressionStatus.totalSteps !== undefined && progressionStatus.totalSteps !== null) ? (
          <div style={{ marginBottom: '4px', fontSize: '13px', color: '#ffffff' }}>
            ...
            {progressionStatus.currentStep || 0} / {progressionStatus.totalSteps || 'N/A'}
          </div>
        ) : null}

        {(progressionStatus.currentEpoch !== undefined &&
          progressionStatus.currentEpoch !== null) ||
        (progressionStatus.totalEpochs !== undefined && progressionStatus.totalEpochs !== null) ? (
          <div style={{ marginBottom: '4px', fontSize: '13px', color: '#ffffff' }}>
            <strong style={{ color: '#e0e0e0' }}>Epochs:</strong>{' '}
            {progressionStatus.currentEpoch || 0} / {progressionStatus.totalEpochs || 'N/A'}
          </div>
        ) : null}

        {progressionStatus.estimatedTimeRemaining && (
          <div style={{ fontSize: '13px', color: '#ffffff' }}>
            <strong style={{ color: '#e0e0e0' }}>Est. Time Remaining:</strong>{' '}
            {Math.round(progressionStatus.estimatedTimeRemaining / 60)} min
          </div>
        )}
      </div>

      {/* Training Metrics - Better organized */}
      {progressionStatus.trainingMetrics &&
        Object.keys(progressionStatus.trainingMetrics).some(
          (key) =>
            ...
            <div style={{ paddingLeft: '8px' }}>
              {progressionStatus.trainingMetrics.loss && (
                <div style={{ marginBottom: '3px', fontSize: '12px', color: '#ffffff' }}>
                  <strong style={{ color: '#e0e0e0' }}>Loss:</strong>{' '}
                  {parseFloat(progressionStatus.trainingMetrics.loss).toFixed(4)}
                </div>
              )}

              {progressionStatus.trainingMetrics.accuracy && (
                <div style={{ marginBottom: '3px', fontSize: '12px', color: '#ffffff' }}>
                  <strong style={{ color: '#e0e0e0' }}>Accuracy:</strong>{' '}
                  {progressionStatus.trainingMetrics.accuracy}
                </div>
              )}
              ...
              {progressionStatus.trainingMetrics.learningRate !== undefined && (
                <div style={{ marginBottom: '3px', fontSize: '12px', color: '#ffffff' }}>
                  <strong style={{ color: '#e0e0e0' }}>Learning Rate:</strong>{' '}
                  {parseFloat(progressionStatus.trainingMetrics.learningRate).toExponential(2)}
                </div>
              )}

              {progressionStatus.trainingMetrics.checkpointsStored && (
                <div style={{ marginBottom: '3px', fontSize: '12px', color: '#ffffff' }}>
                  <strong style={{ color: '#e0e0e0' }}>Checkpoints:</strong>{' '}
                  {progressionStatus.trainingMetrics.checkpointsStored}
                </div>
              )}
            </div>
      ...
      {progressionStatus.metrics && Object.keys(progressionStatus.metrics).length > 0 && (
        <div style={{ marginBottom: '12px' }}>
          <div
            style={{ fontWeight: 'bold', marginBottom: '6px', fontSize: '13px', color: '#e0e0e0' }}
          >
            Additional Metrics:
          </div>
          <div style={{ paddingLeft: '8px' }}>
      ...
      <div style={{ borderTop: '1px solid #444', paddingTop: '8px' }}>
        {progressionStatus.message && (
          <div
            style={{ marginBottom: '6px', fontSize: '12px', fontStyle: 'italic', color: '#cccccc' }}
          >
            {progressionStatus.message}
          </div>
        )}
  ...
  const getProgressColor = (percent: number, suspended = false) => {
  ...
  const CircularProgress = () => {
    console.log(
      `Progress Debug: ${percentageNum}% - dasharray: ${strokeDasharray}, dashoffset: ${strokeDashoffset}`,
    );
