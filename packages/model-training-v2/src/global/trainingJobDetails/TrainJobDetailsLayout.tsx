...
const TrainJobDetailsLayout: React.FC<TrainJobDetailsLayoutProps> = ({ job }) => {
  ...
  const fetchJobsAndPods = React.useCallback(
        ...
        const [jobsData, podsData] = await Promise.all([
          ...
          getPodsForTrainJob(job).catch((error) => {
            ...
            if (error.response?.status === 404 || error.message?.includes('not found')) {
              console.warn(
                'Pods not found for training job, continuing without pods:',
                error.message,
              );
              return [];
            }
  ...
  const getPodRestarts = (pod: PodKind): number => {
    return (
      pod.status?.containerStatuses?.reduce((total, container) => {
...
[2025-09-09 16:07:31] Job completed successfully`;
    ...
    if (jobType === 'dataset-initializer') {
            ...
            <CardBody style={{ maxHeight: '70vh', overflowY: 'auto' }}>
              <Stack hasGutter>
                {jobs.map((jobInfo) => {
                  ...
                  const completedPods = jobInfo.pods.filter(
                    (p) => getPodStatus(p) === 'Succeeded',
                  ).length;
                    ...
                    <StackItem key={jobInfo.job.metadata.name}>
                      <div
                        style={{
                          ...
                          border: isSelected
                            ? '1px solid var(--pf-v5-global--primary-color--100)'
                            ...
                            <CardBody>
                              ...
                              <Stack hasGutter>
                                ...
                                {(jobInfo.job as any).metadata?.creationTimestamp && (
                                  <StackItem
                                    style={{ padding: 'var(--pf-v5-global--spacer--xs) 0' }}
                                  >
                                    <Flex
                                      spaceItems={{ default: 'spaceItemsSm' }}
                                      alignItems={{ default: 'alignItemsCenter' }}
                                ...
                                {(() => {
                                  const volumeMounts =
                                    jobInfo.job.spec?.template.spec.containers[0]?.volumeMounts;
                                  ...
                                  return (
                                    <StackItem
                                      style={{ padding: 'var(--pf-v5-global--spacer--sm) 0' }}
                                    >
                                      <ExpandableSection
                                              ...
                                              <StackItem key={index}>
                                                <div
                                                  style={{
                                                    ...
                                                    borderRadius:
                                                      'var(--pf-v5-global--BorderRadius--sm)',
                                                    border:
                                                      '1px solid var(--pf-v5-global--BorderColor--200)',
                                                  }}
                                                >
                                                  <Flex
                      ...
                      <Card>
                        <CardTitle>
                          <Title headingLevel="h4" size="md">
                            {selectedJob.type.charAt(0).toUpperCase() +
                              selectedJob.type.slice(1).replace('-', ' ')}{' '}
                            : {job.metadata.name}
                          </Title>
                        </CardTitle>
                        <CardBody>
                            ...
                            <Tbody>
                              {selectedJob.pods.map((pod) => {
                                ...
                                return (
                                  ...
                                  >
                                    <Td dataLabel="Pod Name">{pod.metadata.name}</Td>
                      ...
                      <Card>
                        ...
                        <CardBody>
                          <EmptyState>
                            ...
                            <EmptyStateBody>
                              Pods for this job are not available. They may not have been created
                              yet or have been cleaned up.
                            </EmptyStateBody>
                          </EmptyState>
                        </CardBody>
                      </Card>
                      ...
                      <CardTitle>
                        ...
                        >
                          ...
                          {selectedContainer && (
                            <FlexItem>
                              <Label
                                color={selectedContainer.isInitContainer ? 'purple' : 'teal'}
                                isCompact
                              >
                                {selectedContainer.isInitContainer ? 'Init Container' : 'Container'}
                            ...
                            <>
                              ...
                              <FlexItem>
                                <Tooltip
                                  content={
                                    showAllLogs
                                      ? 'Show last 10,000 lines only'
                                      : 'Show all logs (may be slow)'
                                  }
                                >
                                  <Button
                                ...
                                <FlexItem style={{ maxWidth: '250px' }}>
                                  <Dropdown
                                    ...
                                    onOpenChange={(isOpen: boolean) =>
                                      setContainerDropdownOpen(isOpen)
                                    }
                                      ...
                                      >
                                        ...
                                        >
                                          <FlexItem flex={{ default: 'flex_1' }}>
                                            <Truncate
                                              content={
                                                selectedContainer?.name ?? 'Select container'
                                              }
                                            />
                                          </FlexItem>
                            ...
                            >
                              <div style={{ padding: 'var(--pf-v5-global--spacer--md)' }}>
                                <DashboardLogViewer
                                  ...
                                  data={
                                    logsError
                                      ? logsError.message.includes('not found') ||
                                        logsError.message.includes('404')
